...
from gym.envs.robotics import rotations, robot_three_stack_env, utils
...
class FetchEnv(robot_three_stack_env.RobotEnv):
    ...
    def _get_obs(self):
        ...
        dg1 = deepcopy(self.goal)
        dg2 = deepcopy(self.goal)
        dg1[2] += 0.05
        dg2[2] += 0.05 * 2
    ...
    def _reset_sim(self, success_list=[0, 0, 0]):
    ...
    def _is_success(self, obs):
        ...
        d2 = goal_distance(obs['ag2'], obs['dg2'])
        return (d0 < self.distance_threshold and d1 < self.distance_threshold and d2 < self.distance_threshold).astype(
            np.float32)
